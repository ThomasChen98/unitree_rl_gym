...
def pose_default(time_sim, config):
    """静止动作7: 保持默认姿态 - 上身完全不动"""
    upper_body_targets = [
        0.0,  # torso
        0.0,
...
def generate_upper_body_trajectory(time_sim, config, trajectory_type="circles"):
    ...
    trajectory_functions = {
        ...
        "pose_default": pose_default,  # 保持默认姿态
    }
...
def extract_observations(d, default_angles, config, action, time_sim, cmd_values=None):
    ...
    qj = d.qpos[7:34]  # Lower body joints
    dqj = d.qvel[6:33]  # Lower body joint velocities
...
Available trajectory types:
    Static poses: pose_arms_forward, pose_t_shape, pose_default, etc.
    ...
    print("  Static poses: pose_arms_forward, pose_t_shape, pose_default")
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and (time.time() - start) < sim_duration:
            ...
            if counter % control_decimation == 0:
                ...
                default_angles = np.concatenate(
                    [lower_default_angles, upper_default_angles]
                )
