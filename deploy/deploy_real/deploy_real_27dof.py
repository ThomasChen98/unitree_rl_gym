...
from config_27dof import Config
...
class Controller:
    ...
    def move_to_default_pos(self):
        ...
        print(self.config.default_angles)
    ...
    def run(self):
        ...
        qj_reordered = qj_obs[MUJOCO_TO_POLICY].copy()
        self.obs[9 : 9 + num_actions] = qj_reordered

        dqj_reordered = dqj_obs[MUJOCO_TO_POLICY].copy()
        self.obs[9 + num_actions : 9 + 2 * num_actions] = dqj_reordered

        action_reordered_obs = self.action[MUJOCO_TO_POLICY].copy()
        self.obs[9 + 2 * num_actions : 9 + 3 * num_actions] = action_reordered_obs

        # remove last two elements
        obs_tensor = torch.from_numpy(self.obs[:-2]).unsqueeze(0)
        # ------------------------------------------------------------------
        ...
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        # Reorder action to match robot joint order
        action_reordered = self.action[POLICY_TO_MUJOCO].copy()
        self.action = action_reordered
        # ------------------------------------------------------------------
