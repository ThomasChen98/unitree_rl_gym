...
class H1_2FullBodyObsRobot(LeggedRobot):
    ...
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        # 初始化上半身随机目标
        self._randomize_upper_body_targets(
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[63:75] = 0.0  # actions (12)
        noise_vec[75:77] = 0.0  # phase (2)
    ...
    def _randomize_upper_body_targets(self, env_ids):
        ...
        # 为每个重置的环境生成新的随机上半身目标
        random_angles = (
            torch.rand(len(env_ids), 15, device=self.device)
    ...
    def _compute_torques(self, actions):
        ...
        if control_type == "P":
            torques = (
                self.p_gains * (full_actions + self.default_dof_pos - self.dof_pos)
                - self.d_gains * self.dof_vel
            )
        elif control_type == "V":
            torques = (
                self.p_gains * (full_actions - self.dof_vel)
    ...
    def compute_observations(self):
        ...
        self.obs_buf = torch.cat(
            (
                ...
                * self.obs_scales.dof_pos,  # 27维 (全身)
    ...
    def _reward_direction_consistency(self):
        """方向一致性奖励 - 当命令x方向与脚步位移方向一致时奖励"""
        # 计算基座在世界坐标系下的位移方向
        # 使用基座线速度的x分量作为位移方向指示
        displacement_x = self.base_lin_vel[:, 0]  # 前进方向的线速度

        # 获取命令的x方向 (前进命令)
        cmd_x = self.commands[:, 0]

        # 方向一致性检查: 命令方向与实际位移方向符号相同时给奖励
        direction_correct = cmd_x * displacement_x > 0.0

        # 额外奖励: 速度大小与命令大小的匹配程度
        speed_match = torch.exp(
            -torch.abs(torch.abs(displacement_x) - torch.abs(cmd_x))
        )

        # 结合方向正确性和速度匹配度
        reward = direction_correct.float() * speed_match

        return reward

    def _reward_lateral_direction_consistency(self):
        """侧向方向一致性惩罚 - 惩罚y方向与命令不一致的位移"""
        # 计算基座在世界坐标系下的侧向位移
        displacement_y = self.base_lin_vel[:, 1]  # 侧向线速度

        # 获取命令的y方向 (侧向命令)
        cmd_y = self.commands[:, 1]

        # 方向一致性检查: 命令方向与实际位移方向符号相同
        direction_correct = cmd_y * displacement_y > 0.0

        # 速度差异惩罚 - 速度越不匹配惩罚越大
        speed_error = torch.abs(torch.abs(displacement_y) - torch.abs(cmd_y))

        # 当命令为0时，惩罚任何侧向移动
        zero_cmd_penalty = torch.abs(displacement_y) * (torch.abs(cmd_y) < 0.1)

        # 方向错误惩罚
        direction_penalty = (~direction_correct).float() * (
            torch.abs(displacement_y) + torch.abs(cmd_y)
        )

        # 结合不同类型的惩罚
        penalty = torch.where(
            torch.abs(cmd_y) < 0.1,  # 当侧向命令很小时
            zero_cmd_penalty,  # 使用零命令惩罚
            direction_penalty + speed_error,  # 方向错误惩罚 + 速度差异惩罚
        )
